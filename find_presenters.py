...
def identify_presenters(award_name, winner, tweets):
    with open('presenter_regexes.txt', 'r') as file:
    ...
    for r in regexes:
        ...
        for tweet in tweets.values():
            ...
            if match:
                # print("match")
                potential_presenter = match.group(1)
                doc = nlp(tweet.clean_text)
                presenter_entities = [ent.text for ent in doc.ents if ent.label_ in {'PERSON', 'ORG', 'WORK_OF_ART'}]
                for winner in presenter_entities:
                    if winner in potential_presenter:
                        matches[winner] += scoring(potential_presenter, tweet)
